...
BeforeAll {
    . (Join-Path $PSScriptRoot '../scripts/Invoke-EmbedAudio.ps1') -InputPath 'deck.pptx' -AudioDir 'audio/' -OutputPath 'out.pptx'
    Mock Write-Host {}

    # Shared stub directory for python executable
    $script:StubRoot = Join-Path ([System.IO.Path]::GetTempPath()) "pptx-audio-pester-$PID"
...
Describe 'Invoke-EmbedAudio' -Tag 'Unit' {
    BeforeAll {
    ...
    Context 'when python script succeeds' {
        ...
        It 'Completes without error' {
            { Invoke-EmbedAudio } | Should -Not -Throw
        }
    }
    ...
    Context 'when python script fails' {
        ...
        It 'Throws with exit code message' {
            { Invoke-EmbedAudio } | Should -Throw '*embed_audio.py failed*'
        }
    }
}
